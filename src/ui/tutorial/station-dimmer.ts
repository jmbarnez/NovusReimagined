...
export function syncStationDimmerCutout(target: HTMLElement | null): void {
  ...
  const stationRect = getBounds(dimmer);
  if (!target) {
  ...
  setCssText(segments[1], `display:block;left:0;top:${bottom}px;width:100%;height:${Math.max(0, stationRect.height - bottom)}px;`);
  setCssText(segments[2], `display:block;left:0;top:${top}px;width:${left}px;height:${Math.max(0, bottom - top)}px;`);
  setCssText(segments[3], `display:block;left:${right}px;top:${top}px;width:${Math.max(0, stationRect.width - right)}px;height:${Math.max(0, bottom - top)}px;`);
}
